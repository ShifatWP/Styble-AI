...
>
> **The migration has landed.** The current pipeline is:
>
> ```
> prompt
...
>   → validator, 29 codes, never coerces    includes/class-validator.php
...
> - **There is no serializer.** `class-serializer.php` and `class-theme-context.php`
>   are deleted. Styble blocks are dynamic — `save()` returns `InnerBlocks.Content`
>   or `null` — so there is no markup to concatenate, and building blocks in the
>   editor is what makes uniqueId and defaults work.
...
> php scripts/validate.php            # 31 contract fixtures
...
> ```
>
> Still to come: `docs/STYBLE_BLOCKS_MIGRATION_PLAN.md` phases C–F (granular
> per-block edit tools, pattern library, headless pages, MCP). This banner comes
> down when the guide below is rewritten rather than annotated.
