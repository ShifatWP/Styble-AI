...
| 23 | A provider that never reached the validator (no tool call, truncated, malformed JSON) got zero corrective retries — the whole request died on attempt 1 regardless of `max_retries` | **Reasoned from a real comment already in the code**: `styble_ai_malformed_tool_call`'s own message said *"the corrective retry cannot help"*, true only because nothing routed a retry to that branch. Confirmed against `docs/STYBLE_AI_DEMO_COMPARISON.md` — a teammate's parallel `styble-ai-demo` branch hit the identical gap (a provider rejecting a tool call server-side) and already has a working recovery pattern | `class-generator.php` now retries a **named, narrow** set of codes where the model produced something unusable but nothing else was wrong (`no_tool_use`, `truncated`, `bad_json`, `malformed_tool_call`), spending the same one-retry budget the validator path already uses — not an extra call. Deliberately excludes config errors and `styble_ai_api_error` (which bundles 429s), so the "429 handling in the plugin" row in the deferred-work table below is untouched — this is not that, and does not become that. `retries=0` (the eval harness's measurement mode) is bit-for-bit unchanged — verified with a disposable smoke script, not a committed suite (S5 applies) |
